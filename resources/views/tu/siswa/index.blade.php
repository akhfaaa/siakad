<x-dashboard-layout>
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h2 class="text-2xl font-semibold tracking-tight text-[#1d1d1f]">Manajemen Data Siswa</h2>
                <p class="text-[15px] text-[#86868b] mt-1">Total: <span class="font-medium text-[#1d1d1f]">{{ $daftarSiswa->total() }}</span> siswa terdaftar.</p>
            </div>

            <div class="flex items-center gap-3 w-full md:w-auto">
                <form action="{{ route('tu.siswa') }}" method="GET" class="relative w-full md:w-64">
                ...
                <a href="{{ route('tu.siswa.create') }}" class="flex-shrink-0 px-4 py-2.5 rounded-xl bg-[#0071e3] text-white text-[14px] font-semibold hover:bg-[#0077ED] active:scale-[0.98] transition-all duration-200 shadow-sm flex items-center gap-2">
                ...
                <table class="w-full text-left text-[14px] text-[#1d1d1f]">
                    ...
                    <tbody class="divide-y divide-[#d2d2d7]/30">
                        ...
                        <tr class="hover:bg-[#f5f5f7]/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-[13px] text-[#515154]">
                                {{ $siswa->jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan' }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="#" class="p-2 rounded-lg text-[#0071e3] hover:bg-[#f5f5f7] transition-colors border border-transparent hover:border-[#d2d2d7]/50" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <form action="#" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 rounded-lg text-red-500 hover:bg-red-50 transition-colors border border-transparent hover:border-red-100" title="Hapus" onclick="return confirm('Yakin ingin menghapus data siswa ini?')">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
